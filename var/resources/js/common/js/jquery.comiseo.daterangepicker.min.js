...
function formatRangeForDisplay(range){var dateFormat=options.dateFormat;return $.datepicker.formatDate(dateFormat,range.start)+options.rangeSplitter+$.datepicker.formatDate(dateFormat,range.end)}
...
function parseRange(text){var dateFormat=options.altFormat,range=null;if(text)try{range=JSON.parse(text,function(key,value){return key?$.datepicker.parseDate(dateFormat,value):value})}catch(e){};return range}
...
function setRange(value,event){var range=value||calendar.getRange();if(!range.start)return;if(!range.end)range.end=range.start;value&&calendar.setRange(range);triggerButton.setLabel(formatRangeForDisplay(range));$originalElement.val(formatRangeForDisplay(range)).change();if(options.onChange)options.onChange();instance._trigger('change',event,{instance:instance})}
function getRange(){return parseRange($originalElement.val())}
function clearRange(event){triggerButton.reset();calendar.reset();if(options.onClear)options.onClear();instance._trigger('clear',event,{instance:instance})}
